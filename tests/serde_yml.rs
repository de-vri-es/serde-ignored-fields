...
fn yaml<T: serde::de::DeserializeOwned>(data: &str) -> Result<PreverveIgnoredFields<T, serde_yml::Mapping>, serde_yml::Error> {
...
macro_rules! yaml {
	...
	([ $($value:tt),* $(,)? ]) => {
		{
			#[allow(unused_mut)]
			let mut vec = Vec::<serde_yml::Value>::new();
...
fn serialize_extra() {
	...
	let_assert!(Ok(serialized) = ::serde_yml::to_string(&zohan));
